...
1. **`AGENTS.md`** at repo root — lean session-bootstrap (~180 lines).
...
├── claude_session.rs       Spawn `Codex` CLI cross-repo bug fixing
...
│   │                       switches a single test to the Codex subprocess
...
.Codex/skills/
├── sirin-launch/           For external Codex sessions starting Sirin
├── sirin-test/             For external Codex sessions running tests
...
**parking_lot → std::sync migration rule** (AGENTS.md enforced):
...
3. **`src/mcp_server.rs`** — add `browser_exec` action arm so
   external Codex sessions can call it via MCP:
   ```rust
   "clear_state" => {
       browser::clear_browser_state()?;
       Ok(json!({ "status": "cleared" }))
...
5. **`.Codex/skills/sirin-test/SKILL.md`** + **`docs/MCP_API.md`** —
...
to spawning `Codex -p` per LLM call. Iteration 1 works (~6s, small
...
with `Codex subprocess timed out after 600s`.
...
Windows. The hang is inside the `Codex` CLI itself, not in our wrapper.
...
**Fix needed:** investigate whether `Codex` CLI has its own bug with
...
- After every feature commit: update `.Codex/skills/sirin-*/SKILL.md`
