                            ...
                            _ => {
                                    ...
                                    listeners.lock().unwrap().get(&ListenerId::Browser)
                                {
                                    if let Err(err) = tx.send(browser_event.clone()) {
                                        ...
                                        // PATCH (Sirin): do NOT break here.
                                        // A SendError means the browser event loop's receiver
                                        // was dropped (e.g. it timed out), but the underlying
                                        // WebSocket connection is still alive.  Breaking kills
                                        // the whole transport, which cascades into a Chrome
                                        // crash for every subsequent CDP call.  Instead, just
                                        // continue — the browser event loop will be restarted
                                        // by Sirin's recovery code if needed.
                                        // Original: break;
                                        continue;
                                    }
                                }
