...
class FakeWDA(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: vulture
        ...
        elif "/element/" in self.path and self.path.endswith("/click"):
            if self._session_dead():
                return
            self._reply(None)
...
def test_element_click_uses_the_selected_element_id(wda):
    wda.element_click("remote-desktop-icon")
    assert (
        "POST",
        f"/session/{wda.session_id}/element/remote-desktop-icon/click",
    ) in FakeWDA.requests_seen
