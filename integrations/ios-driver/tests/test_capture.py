...
def test_go_ios_screenshot_decodes_utf8_with_replacement(monkeypatch):
    seen = {}

    def fake_run(args, **kwargs):
        seen.update(kwargs)
        Path(args[args.index("--output") + 1]).write_bytes(b"png")
        return type("Proc", (), {"stderr": ""})()

    monkeypatch.setattr(capture.device, "ios_path", lambda: r"D:\tools\ios.exe")
    monkeypatch.setattr(capture.subprocess, "run", fake_run)
    assert capture._go_ios_screenshot() == b"png"
