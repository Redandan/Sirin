...
fn team_cwd() -> String {
    crate::claude_session::repo_path("sirin").unwrap_or_else(|| {
        std::env::current_dir()
    ...
    let (pm, engineer, tester) = {
        ...
        match guard.as_ref() {
            Some(team) => {
                ...
                (
                    to_member_view(&s.pm),
                    to_member_view(&s.engineer),
                    to_member_view(&s.tester),
                )
            }
            None => (
                blank_member("pm"),
                blank_member("engineer"),
                blank_member("tester"),
            ),
        }
    };
...
pub fn dev_team_enqueue_issue(
    ...
    if dry_run {
        ...
        let _ = priority;
        multi_agent::github_adapter::enqueue_from_issue_dry_run(project_key, gh_repo, issue_number)
    } else {
