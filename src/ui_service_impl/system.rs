...
pub(super) fn log_version(_svc: &RealService) -> usize {
...
pub(super) fn log_len(_svc: &RealService) -> usize {
    crate::log_buffer::len()
}

pub(super) fn log_clear(_svc: &RealService) {
...
fn system_status_cache() -> &'static std::sync::Mutex<(std::time::Instant, Option<SystemStatus>)> {
    static CACHE: std::sync::OnceLock<
        std::sync::Mutex<(std::time::Instant, Option<SystemStatus>)>,
...
pub(super) fn system_status(svc: &RealService) -> SystemStatus {
    ...
    let v = SystemStatus {
        ...
        mcp_tools: crate::mcp_client::get_discovered_tools()
            .iter()
            .map(|t| McpToolInfo {
...
pub(super) fn available_models(_svc: &RealService) -> Vec<String> {
    ...
    });
    {
        ...
        if !g.1.is_empty() && g.0.elapsed() < TTL {
            return g.1.clone();
        }
    }
    let fleet = crate::llm::shared_fleet();
    let v: Vec<String> = fleet
        .classified_models
        .iter()
...
pub(super) fn export_config(_svc: &RealService) -> String {
    std::fs::read_to_string(crate::platform::config_path("agents.yaml"))
        .unwrap_or_else(|_| "# empty".to_string())
}

pub(super) fn import_config(svc: &RealService, yaml: &str) -> Result<(), String> {
    let _: crate::agent_config::AgentsFile =
        serde_yaml::from_str(yaml).map_err(|e| format!("YAML 解析失敗: {e}"))?;
    std::fs::write(crate::platform::config_path("agents.yaml"), yaml)
...
pub(super) fn config_apply_fixes(
    svc: &RealService,
    fixes: Vec<ConfigFixView>,
) -> Result<Vec<String>, String> {
    let core_fixes: Vec<crate::config_check::ConfigFix> = fixes
        .into_iter()
        .map(|f| crate::config_check::ConfigFix {
            file: f.file,
            field_path: f.field_path,
            current_value: f.current_value,
            new_value: f.new_value,
    ...
    svc.push_toast(
        ToastLevel::Success,
        format!("已套用 {} 項配置修改（重啟生效）", applied.len()),
    );
    Ok(applied)
}
