...
pub(super) fn teams_running(_svc: &RealService) -> bool {
    !matches!(
        crate::teams::session_status(),
        crate::teams::SessionStatus::NotStarted
    )
}
...
pub(super) fn mcp_tools(_svc: &RealService) -> Vec<McpToolDetail> {
    ...
    let v: Vec<McpToolDetail> = crate::mcp_client::get_discovered_tools()
        .iter()
        .map(|t| {
            let params = t
                .input_schema
...
pub(super) fn mcp_call(
    ...
    let tool = tools
        .iter()
        .find(|t| t.registry_name() == tool_name || t.tool_name == tool_name)
    ...
    let rt = tokio::runtime::Handle::try_current().map_err(|_| "No tokio runtime".to_string())?;
    let result =
        std::thread::spawn(move || rt.block_on(crate::mcp_client::call_tool(&url, &name, args)))
            .join()
...
pub(super) fn sirin_mcp_call(
    _svc: &crate::ui_service_impl::RealService,
    tool_name: &str,
    args_json: &str,
) -> Result<String, String> {
...
pub(super) fn chat_send(svc: &RealService, _agent_id: &str, message: &str) -> String {
    ...
    if let Ok(rt) = tokio::runtime::Handle::try_current() {
        ...
        .join()
        .unwrap_or_else(|_| "（Agent 回覆失敗）".to_string())
    } else {
...
pub(super) fn execute_skill(_svc: &RealService, skill_id: &str, input: &str) -> String {
    if let Err(error) = crate::skills::guard_generic_skill_execution(skill_id) {
        return format!("錯誤: {error}");
    }
    ...
    if script_path.exists() {
        return crate::rhai_engine::run_rhai_script(
