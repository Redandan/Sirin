...
mod tests {
    ...
    // utility that pushed 3 sample tasks into the production queue, one
    // of which referenced src/ui_egui/workspace.rs (deleted in v0.5.0).
    // The other two tasks were transient clippy / regex fixes that have
    // already been resolved. If we need to seed the queue again, write
    // a new helper targeting the current code layout.)
}
