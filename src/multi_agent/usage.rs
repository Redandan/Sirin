...
pub fn snapshot(window_secs: u64) -> UsageSnapshot {
    let cell = CACHE
        .get_or_init(|| Mutex::new((Instant::now() - Duration::from_secs(60), Default::default())));
...
pub fn compute(window_secs: u64) -> UsageSnapshot {
    ...
    for sid in &session_ids {
        ...
        for line in text.lines() {
            ...
            snap.input_tokens += usage
                ...
                .and_then(|v| v.as_u64())
                .unwrap_or(0);
            snap.output_tokens += usage
                .get("output_tokens")
                .and_then(|v| v.as_u64())
                .unwrap_or(0);
            snap.cache_read += usage
                .get("cache_read_input_tokens")
                .and_then(|v| v.as_u64())
                .unwrap_or(0);
            snap.cache_write += usage
    ...
    snap.tokens_per_min = if window_secs > 0 {
        snap.total_tokens * 60 / window_secs
    } else {
        0
    };
    ...
    snap.cost_usd = (snap.input_tokens as f64 * PRICE_INPUT
        ...
        + snap.cache_write as f64 * PRICE_CACHE_WRITE)
        / m;
...
mod tests {
    ...
    fn cost_formula_one_million_each() {
        ...
        let m = 1_000_000.0_f64;
        snap.cost_usd =
            (snap.input_tokens as f64 * PRICE_INPUT + snap.output_tokens as f64 * PRICE_OUTPUT) / m;
