...
"#).unwrap();
    ...
    /// Issue #189: ensure no YAML uses clear_state without a subsequent goto ?__test_role=
    /// when the test URL itself contains ?__test_role=.
    ///
    /// Root cause: clear_state wipes localStorage → Flutter re-routes to #/login and
    /// drops the URL's ?__test_role= param → auto-login doesn't trigger.
    ///
    /// Only applies to tests whose URL has ?__test_role= (login-dependent tests).
    /// Tests without ?__test_role= (e.g. smoke tests testing the login page) are exempt.
    #[test]
    fn agora_regression_yamls_clear_state_followed_by_reauth_goto() {
        let all = load_all();
        let mut violations: Vec<String> = Vec::new();

        for test in &all {
            // Only lint tests that rely on __test_role auto-login
            if !test.url.contains("__test_role=") {
                continue;
            }
            ...
            for (i, step) in steps.iter().enumerate() {
                if step.to_lowercase().contains("clear_state") {
                    let next_5 = &steps[i.saturating_add(1)..steps.len().min(i + 6)];
                    let has_reauth = next_5.iter().any(|s| {
                        let lower = s.to_lowercase();
                        lower.contains("goto") && lower.contains("__test_role=")
        ...
        assert!(
            violations.is_empty(),
            "Found {} YAML(s) with clear_state not followed by goto ?__test_role= (#189):\n{}",
            violations.len(),
            violations.join("\n")
        );
    }
