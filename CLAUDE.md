...
| `LLM_PROMPT_CACHE` | `1`/`true`/`yes` (default off) | Issue #260: opt-in to Anthropic's native `/v1/messages` endpoint with `cache_control` markers on the system prompt.  Only takes effect when `LLM_PROVIDER=anthropic` AND `LLM_BASE_URL` (or default) targets `anthropic.com` — proxies / OpenRouter stay on OpenAI-compat path.  When on, the **first** call writes the system prompt to ephemeral cache (charged at +25% premium for that block once); subsequent calls within ~5 minutes pay only a 10% cache-read rate, surfaced as `cached_tokens` on the per-run dashboard.  Net win: ≥ 50% input-token reduction on repeated ReAct iterations of the same test. |
