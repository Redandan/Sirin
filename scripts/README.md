...
| **`switch-sirin-daemon.ps1`** | Verifies an exact Sirin candidate on an alternate loopback port, guards against MCP tool regression, preserves the scheduled-task action, and supports immutable hash-versioned deployment directories with automatic rollback. | Before switching the task-owned Windows daemon; run `Status` or `Verify` first, supply the reviewed SHA-256, and pass `-DeploymentRoot` to deploy without overwriting the running or prior binary. |
