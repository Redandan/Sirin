...
| **`switch-sirin-daemon.ps1`** | Verifies an exact Sirin candidate on an alternate loopback port, guards against MCP tool regression, and preserves the scheduled-task action for deploy or explicit rollback. | Before replacing the task-owned Windows daemon; run `Status` or `Verify` first and supply the reviewed SHA-256 for any candidate. |
