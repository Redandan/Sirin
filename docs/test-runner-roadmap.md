...
> v1 — 2026-04-16, **revised after author's review**.
> 作者回饋後修正 3 處:移除 2.2(Bayesian 過度工程)、1.2 估時從 1.5hr 改 0.5hr(已有 YAML 欄位)、1.1 估時從 2hr 改 3-4hr(3 個 prompt)、修「繁中亂碼」用詞不準確。
>
> Analysis based on `6860a82 feat: test_runner external MCP API + async polling + resilience`.
>
> Context: external Claude Code sessions can now invoke Sirin via MCP (`list_tests` / `run_test_async` / `get_test_result` / `get_screenshot` / `get_full_observation`). Async polling works. Parse-fail reprompt works. This roadmap covers what's *still* missing before Sirin is a drop-in replacement for one-off Puppeteer/Playwright scripts.

---

## P1 — Blocks non-Chinese users (1-3 hr each)

### 1.1 Locale-aware prompts
...
### 1.2 `retry_on_parse_error` YAML 欄位
...
### 1.3 `.claude/skills/sirin-test.md` 說明
...
### 2.1 Observation truncation — 讓 LLM 自主 pull 完整內容
