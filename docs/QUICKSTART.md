...
**中文顯示為方框**（v0.5.0 之前的 egui UI 才有此問題）
v0.5.0+ 改用瀏覽器 web UI，字型由系統提供，不再會缺字。如果還是有問題請確認瀏覽器有安裝中文字型。

**瀏覽器沒自動開啟**
Sirin 會嘗試打開系統預設瀏覽器到 `http://127.0.0.1:7700/ui/`。失敗時手動打開即可。要關閉自動打開：`SIRIN_HEADLESS=1 ./sirin`。
